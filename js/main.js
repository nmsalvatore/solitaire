...
function updateThemeColor(theme) {
    ...
    if (meta)
        meta.setAttribute("content", THEME_COLORS[theme] || THEME_COLORS.blue);
}
...
document.addEventListener("keydown", (e) => {
    ...
    const focusable = activeTrap.querySelectorAll(
    ...
    for (let ci = 0; ci < 7; ci++) {
        ...
        if (idx !== -1) {
            ...
            return {
                card: col[idx],
                cards,
                source: { type: "tableau", colIndex: ci },
            };
        }
    }
...
function createTouchGhost(touch) {
    ...
    if (source.type === "waste") {
        ...
        pileEl = document.querySelector(
            `.foundation[data-suit="${cards[0].suit}"]`,
        );
    } else {
        pileEl = document.querySelector(
            `.tableau-col[data-col="${source.colIndex}"]`,
        );
    }
    if (!pileEl) return;
    ...
    const firstCardEl = pileEl.querySelector(
        `.card[data-suit="${firstCard.suit}"][data-rank="${firstCard.rank}"]`,
    );
    ...
    if (cards.length > 1) {
        ...
        const lastCardEl = pileEl.querySelector(
            `.card[data-suit="${lastCard.suit}"][data-rank="${lastCard.rank}"]`,
        );
        if (lastCardEl) {
    ...
    cards.forEach((card) => {
        const cardEl = pileEl.querySelector(
            `.card[data-suit="${card.suit}"][data-rank="${card.rank}"]`,
        );
        if (!cardEl) return;
...
function handleDragStart(e) {
    ...
    if (!cardEl || cardEl.dataset.faceUp !== "1") {
        e.preventDefault();
        return;
    }
    ...
    e.dataTransfer.setData(
        "text/plain",
        `${cardEl.dataset.suit}:${cardEl.dataset.rank}`,
    );
...
function handleDragOver(e) {
    ...
    if (pileEl.classList.contains("foundation")) {
        if (dragState.cards.length === 1) {
            const fi = Game.SUITS.indexOf(pileEl.dataset.suit);
            valid = Game.canMoveToFoundation(
                dragState.cards[0],
                state.foundations[fi],
            );
        }
...
function handleDrop(e) {
    ...
    if (pileEl.classList.contains("foundation")) {
        if (dragState.cards.length === 1) {
            ...
            if (
                Game.canMoveToFoundation(card, Game.getState().foundations[fi])
            ) {
                Game.saveSnapshot();
        ...
        if (lazyMode) {
            ...
            if (dest) {
                ...
                if (dest.type === "foundation") {
                    moved = Game.moveToFoundation(
                        resolved.card,
                        resolved.source,
                    );
                    if (moved && Game.checkWin()) showWin();
                } else {
                    moved = Game.moveToTableau(
                        resolved.cards,
                        dest.colIndex,
                        resolved.source,
                    );
...
function tryDrop(pileEl) {
    ...
    if (pileEl.classList.contains("tableau-col")) {
        const colIndex = parseInt(pileEl.dataset.col);
        Game.saveSnapshot();
        const moved = Game.moveToTableau(
            selection.cards,
            colIndex,
            selection.source,
        );
...
(function HelpSlider() {
    ...
    function goTo(n) {
        ...
        document
            .querySelectorAll(".help-slide")
            .forEach((s, i) => s.classList.toggle("active", i === cur));
...
document.addEventListener("keydown", (e) => {
    if (
        e.key === "Escape" &&
        !document.getElementById("win-screen").hasAttribute("hidden")
    ) {
        hideWin();
        return;
    }
...
document.addEventListener("keydown", (e) => {
    if (
        !document.getElementById("confirm-screen").hasAttribute("hidden") &&
        e.key === "Escape"
    ) {
        hideConfirm();
    }
